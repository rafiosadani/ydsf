...
//
//penulisan untuk format json: (untuk menampilkan data saja)
//$json_pegawai='{ "name":"John", "age":31, "city":"New York" }';
// $data['formtahun'][] = array('id' => 'tahunaktif', 'label' => 'SKP Tahun :', 'type' => 'json', 'value' => $json_pegawai, 'validation' => 'required|numeric|min_length[4]|max_length[4]');
...
/*
  ...
  2. Load this library                    ---         $this->load->library('formbuilder');
...
class Formbuilder {
    ...
    function build_form_horizontal($options, $data_source = array()) {
        ...
        foreach ($options as $elm_options) {
            ...
            //unset validation form CI validation
            unset($elm_options['validation']);
...
//        }
        ...
        if ($this->func == 'form_combine') {
            ...
            switch ($this->func) {
                ...
                case 'form_editor':
                    $this->elm_options['value'] = html_entity_decode($this->elm_options['value']);
                    $input_html_string = form_textarea($this->elm_options);
                    $input_html_string.= "<script type='text/javascript'>CKEDITOR.replace('".$this->elm_options['id']."');</script>";
                    break;
                case 'form_file':
    ...
    private function _build_help_block() {
        if (!empty($this->elm_options_help)) {
            return '<small class="help-block text-danger">' . $this->elm_options_help . '</small>';
        }
        return '';
    }
