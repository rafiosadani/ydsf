...
class M_donatur extends CI_Model {
    ...
    public function insertDonatur($last)
    {
        ...
        $object = array(
            ...
            'entri_pegawai' => $this->session->userdata('username'),
    ...
    {
        $last_id = $this->db->query("select kwsn from kawasan ORDER BY kwsn DESC limit 1")->row();
    ...
    public function keyDonatur($keyword,$offset) {
        $this->db->select('count(autoid) as tt_data, donaturbaru.almktr as almktr, donaturbaru.telphp as telphp , donaturbaru.jupen, donaturbaru.kwsn, donaturbaru.noid, donaturbaru.nama, donaturbaru.autoid, donaturbaru.status, donaturbaru.alamat, program.NM_PROGRAM, donatur_item.besar, kawasan.kodejgt');
        $this->db->from('donaturbaru');
        $this->db->join('donatur_item', 'autoid = donatur_item.noid', 'INNER');
        $this->db->join('program', 'donatur_item.prog = program.PROG', 'INNER');
        $this->db->join('kawasan', 'donaturbaru.kwsn = kawasan.kwsn', 'INNER');
    ...
    public function keyDonaturCab($keyword,$offset) {
        $this->db->select('count(autoid) as tt_data,donaturbaru.kwsn, donaturbaru.noid, donaturbaru.nama, donaturbaru.autoid, donaturbaru.status, donaturbaru.alamat, program.NM_PROGRAM, donatur_item.besar, kawasan.kodejgt');
        $this->db->from('donaturbaru');
        $this->db->join('donatur_item', 'autoid = donatur_item.noid', 'INNER');
        $this->db->join('program', 'donatur_item.prog = program.PROG', 'INNER');
        $this->db->join('kawasan', 'donaturbaru.kwsn = kawasan.kwsn', 'INNER');
